...
class Network(nn.Module):
    ...
    def __init__(self):
        ...
        self.self_attn0 = nn.MultiheadAttention(
            p.d_attn, p.num_heads, batch_first=True)
        self.layer_norm0 = nn.LayerNorm(p.d_attn)
        ...
        self.cross_attn4 = nn.MultiheadAttention(
            p.d_attn, p.num_heads, batch_first=True)
    ...
    def forward(self, x_part, view):
        ...
        x, _ = self.self_attn0(pc_feat, pc_feat, pc_feat)
        pc_feat = self.layer_norm0(x + pc_feat) # B x N x F
        ...
        x, _ = self.cross_attn4(pc_feat, im_feat, im_feat)
