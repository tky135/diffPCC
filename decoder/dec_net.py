...
class Decoder_Network(nn.Module):
    def __init__(self):
        ...
        self.pointgen = nn.ModuleList([AXform(self.K1, self.K2, self.N, i) for i in range(self.num_branch)])
        
    def forward(self, x, x_part):
        # x_part is the input PPC (Not used)
        # x is the feature map from the encoder
      
        x_feat = x

        x_1 = torch.empty(size=(x_part.shape[0], 0, 6)).to(x_part.device)
...
if __name__ == '__main__':
    ...
    axform = AXform(opt.K1, opt.K2, opt.N).cuda()
    output = axform(x)
    print(output.shape)
